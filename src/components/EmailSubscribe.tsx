...
const SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbzVbMfZUUIH8-vp19-SxiNbMtCDu4qSpf0vqMFg7NhVG725YcxUr9YvPyeeEzwDcLpdpA/exec";
...
const EmailSubscribe: React.FC<EmailSubscribeProps> = ({ language }) => {
  ...
  return (
    ...
    >
      ...
      {/* Subtitle */}
      <p
        className="text-xs text-white/40 mb-3 leading-relaxed"
        dir={isRTL ? "rtl" : "ltr"}
      >
        {labels.subtitle}
      </p>
      ...
      {status === "success" ? (
        ...
        <>
          ...
          {status === "error" && (
            <p
              className="text-[10px] text-[#C8102E] mb-2 px-1"
              dir={isRTL ? "rtl" : "ltr"}
            >
              {labels.error}
            </p>
